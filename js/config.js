...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 2. 只有当 PROXY_URL 存在且不为空时，才启用备用代理
            // 修复关键：如果 PROXY_URL 为空或 //，就不要生成错误的备用链接
            let onerrorStr = 'this.onerror=null;';
            if (typeof PROXY_URL !== 'undefined' && PROXY_URL && PROXY_URL.trim() !== '' && PROXY_URL !== '//') {
                 const proxiedCoverUrl = PROXY_URL + encodeURIComponent(originalCoverUrl);
                 onerrorStr += `this.src='${proxiedCoverUrl}'; this.classList.add('object-contain');`;
            }
